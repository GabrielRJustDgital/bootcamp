...
* so they know who is attendin their events.
*/
...
class ReportController extends ControllerBase {  
   ...
   * These are returned as an associate array, with each row
  ...
  protected function load() {
  	try {
  	  ...
  	  $select_querry = $database->select('rsvplist', 'r');

  	  // join the user table, so we can get the entry creator's username.
  	  $select_querry->join('users_field_data', 'u', 'r.uid = u.uid');
  	  // Join the node table, so we can get the event's name.
  	  $select_querry->join('node_field_data', 'n', 'r.nid = n.nid');

  	  // Select these especific fields for the output.
  	  $select_querry->addField('u', 'name', 'username');
  	  $select_querry->addField('n', 'title');
  	  $select_querry->addField('r', 'mail');
  	  ...
  	  $entries = $select_querry->execute()->fetchAll(\PDO::FETCH_ASSOC);
  ...
  public function report() {
  	...
  	// Because load() return an associative array with each table row 
  	// as its own array, ew can sumply define the html table rows like this:
    ...
    // Create the render array for redering an HTML table.
    $content['table'] = [
    	'#type' => 'table',
    	'#header' => $headers,
    	'#rows' => $table_rows,
